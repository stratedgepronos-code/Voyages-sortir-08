    ...
    <div class="vc-inner">
        ...
        <div class="vc-calc-col">
            <div class="vc-calc-card">
                ...
                <div class="vc-calc-hint">Étape 1 : choisissez votre aéroport → les dates s'afficheront ensuite.</div>

                <!-- 1. AÉROPORT (obligatoire en premier) -->
                <div class="vc-field">
                    <label>1. Aéroport de départ <span class="vc-required-dot">*</span></label>
                ...
                <!-- 2. DATE DE DÉPART (activé après aéroport) -->
                <div class="vc-field vc-step-date" id="vc-field-date-wrap">
                    <label>2. Date de départ <span class="vc-required-dot">*</span></label>
                    <p class="vc-date-hint" id="vc-date-hint">Choisissez d'abord un aéroport ci-dessus.</p>
                    <select id="vc-date-depart" disabled style="display:none">
                                ...
                                if ($current > time()) {
                                    ...
                                    if (in_array($dow, $jours_ok)) {
                                        ...
                                        if (!isset($generated_dates[$date_str])) {
                                            $generated_dates[$date_str] = ['date' => $date_str, 'supp' => $supp, 'statut' => $statut];
                                        }
                                    }
                                }
                        ...
                        ?>
                        <option value="<?php echo esc_attr($gd['date']); ?>"><?php echo esc_html(($jours_noms[$dow] ?? '') . '. ' . date('d/m/Y', $ts) . $supp_txt . $garanti); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="vc-price-result" id="vc-price-result">
                    <div style="text-align:center;color:#aaa;font-size:13px;font-family:'Outfit',sans-serif;padding:12px 0">
                        Sélectionnez un aéroport et une date<br>pour voir le prix
                    </div>
                </div>
                ...
                <div class="vc-reassurance">
                    <div class="vc-reass-item">✅ Acompte <?php echo intval($m['acompte_pct'] ?? 30); ?>% seulement</div>
                    <div class="vc-reass-item">🔒 Paiement sécurisé 3D Secure</div>
                    <div class="vc-reass-item">📞 03 26 65 28 63</div>
                </div>
            </div>

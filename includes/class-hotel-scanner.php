...
PROMPT;
        ...
        $data = self::sanitize_hotel_text_fields($data);
...
- Tous les champs texte (nom, desc, chambres.xxx.desc, etc.) doivent être en texte brut uniquement : ne jamais inclure de balises HTML ni de références type <cite> dans le JSON.
PROMPT;
    ...
    /**
     * Supprime récursivement toutes les balises HTML des données hôtel (réponses IA).
     * Évite que des balises type <cite index="12-9"> apparaissent dans les chambres, descriptions, etc.
     */
    private static function sanitize_hotel_text_fields($data) {
        if (!is_array($data)) {
            return is_string($data) ? wp_strip_all_tags($data) : $data;
        }
        $out = [];
        foreach ($data as $k => $v) {
            if (is_string($v)) {
                $out[$k] = wp_strip_all_tags($v);
            } elseif (is_array($v)) {
                $out[$k] = self::sanitize_hotel_text_fields($v);
            } else {
                $out[$k] = $v;
            }
        }
...
Tous les champs texte doivent être en texte brut uniquement : ne jamais inclure de balises HTML ni de références (<cite>, etc.) dans le JSON.';
    }

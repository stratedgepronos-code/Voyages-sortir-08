...
add_action('wp_enqueue_scripts', function() {
    ...
    wp_enqueue_style('vs08-main', get_template_directory_uri() . '/assets/css/main.css', ['vs08-fonts'], '1.4');
    wp_enqueue_style('vs08-header-mega', get_template_directory_uri() . '/assets/css/header-mega.css', ['vs08-main'], '1.4');
        ...
        foreach ($ids as $pid) {
            ...
            if (!empty($m['aeroports']) && is_array($m['aeroports'])) {
                foreach ($m['aeroports'] as $a) {
                    ...
                    if (class_exists('VS08V_Search') && method_exists('VS08V_Search', 'normalize_airport')) {
                        $code = VS08V_Search::normalize_airport($code);
                    }
                    $ville = (class_exists('VS08V_Search') && method_exists('VS08V_Search', 'airport_name')) ? VS08V_Search::airport_name($code) : trim($a['ville'] ?? '');
                    if (!$code) continue;

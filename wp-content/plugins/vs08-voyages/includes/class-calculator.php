...
class VS08V_Calculator {
    ...
    public static function calculate($voyage_id, $params) {
        ...
        if ($prix_gf_total > 0 && $nb_nongolfeurs > 0) {
            $reduction_pct = floatval($m['reduction_nongolfeur'] ?? 30);
            $prix_nongolf_unit = $prix_gf_total * (1 - $reduction_pct / 100);
            $prix_nongolf_total = $prix_nongolf_unit * $nb_nongolfeurs;
            if ($prix_nongolf_total > 0) {
                $result['lines'][] = ['label' => '👤 Accompagnants (-'.$reduction_pct.'% green fees)', 'montant' => $prix_nongolf_total, 'detail' => round($prix_nongolf_unit, 2).'€ × '.$nb_nongolfeurs.' pers.'];
            }
        }
